...
sistema_paquetes = {
    "debian": {"instalar": ["apt", "install", "-y"], "comprobar": ["dpkg", "-s"]},
    "arch": {
...
def instalado(nombre_paquete):
    try:
        ...
        subprocess.run(
            comando, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
        )
...
def comprobar(comando):
    try:
        ...
        if salida.stdout.strip() == str(
            comando[1]
        ):  # Comprueba si la salida es igual a lo esperado
            ejecutar_comando(comando[2])  # En caso afirmativo ejecuta el comando
        elif (
            len(comando) == 4
        ):  # Comprueba la longitud, caso de ser 4 ejecuta el otro parametro
            ejecutar_comando(comando[3])
    except (
        subprocess.CalledProcessError
    ) as error:  # Captura la excepcion que lanza subprocess y muestra un mensaje
        print(
            Colores.ROJO
            + "Error al hacer la comprobacion,  codigo de error: "
            + str(error.returncode)
            + Colores.FINC
        )
        tipo_error.comprobar(
            1
...
def comprobar_comando(comando):
    try:
        ...
        if salida.returncode == int(
            comando[1]
        ):  # Comprueba si el codigo de ejecucion es igual al esperado
            ejecutar_comando(comando[2])  # En caso afirmativo ejecuta el comando
        elif (
            len(comando) == 4
        ):  # Comprueba la longitud, caso de ser 4 ejecuta el otro comando
            ejecutar_comando(comando[3])
...
def ejecutar_comando(comando):
    try:
        com = None
        if (
            type(comando) is list
        ):  # Comprueba si se le ha pasado una lista de comandos o un comando solo
            for com in comando:  # Recorre la lista
                subprocess.run(
                    com, shell=True, check=True, stderr=subprocess.DEVNULL
                )  # Ejecuta el comando
        elif type(comando) is str:  # Comprueba si comando es un string
            subprocess.run(
                comando, shell=True, check=True, stderr=subprocess.DEVNULL
            )  # Ejecuta el comando
        ...
        print(
            ...
            + Colores.AMARILLO
            + (comando if com is None else com)
            + Colores.ROJO
...
def comandos(comando):
    if type(comando) is str and not instalado(
        comando
    ):  # Comprueba si el parametro es un string y si el paquete no esta instalado
        ...
        for llave in comando:  # Recorre parametro
            if llave == "paquete" and not instalado(
                comando[llave]
            ):  # Comprueba si la clave es "paquete" y si no esta instalado el paquete
                ...
                llave == "comprobar_comando"
            ):  # Comprueba si la clave es igual a "comprobar_comando"
...
def cargar_paquete():
    ...
    # Comprueba si el id de la distribucion se encuentra en el objeto de sistema_paquetes
        ...
        print(
            Colores.AZUL
            + "El sistema de archivos de la distribucion"
            + Colores.VERDE
            + distro_id()
            + Colores.AZUL
            + " no esta soportado"
...
def comprobar_dependencias(dependencias):
    ...
    # Comprueba si dependencias es un list
        ...
        dependencias is None
    ):  # En caso de que dependencias no tenga valor, no devuelve ningun valor
        ...
        raise Exception(
            "Error: el tipo de dependencias ha de ser un listado o un string"
        )  # Lanza una excepcion
...
def renderizar_variables(
    json, dependencias=None, json_variables=None, contiene_variables=None
):
    ...
    if json_variables is None and contiene_variables is None:
        renderizar = archivo_config[
            "contiene_variables"
        ]  # Asigna a renderizar el valor de contiene_variables de archivo_config
    # Si el archivo tiene variables declaradas y contiene_variables es nulo, se renderizaran las variables
        ...
        renderizar = (
            contiene_variables  # Asigna a renderizar el valor de contiene_variables
        )
    ...
    if renderizar == True:  # (En caso de que el valor sea 1 tambien entrará)
        ...
        datos_renderizado.update(
            json_variables or {}
        )  # En caso de que alguna propiedad se repita las variables locales la sobre
...
def archivo_configuracion(archivo):
    ...
    if not archivo_config[
        "iniciado"
    ]:  # Comprueba si ya se ha pasado antes otro archivo de configuracion
        ...
        if archivo.get("sudo") == True and geteuid != 0:
            print(
                Colores.ROJO + "Tienes que ser super usuario" + Colores.FINC
            )  # Muestra un mensaje indicando que es necesario ser usuario root
            exit(1)  # Sale con el codigo de error 1

        # Comprueba si el archivo es de configuracion
        if (
            paquete is None and archivo.get("contiene_instalar") != False
        ):  # Contiene_instalar permite usar los comandos sin necesidas de cargar el sistema de paquetes
            if (
                not archivo.get("distro") is None
            ):  # Comprueba si la distro esta declarada en el archivo
                paquete = sistema_paquetes.get(
                    archivo.get("distro")
                )  # Carga el sistema de paquetes apartir de la distro
            else:
        ...
        if not archivo.get("finalizar_error") is None:
            ...
            archivo_config[
                "finalizar_error"
            ] = False  # Establece finalizar_error en la configuracion global para que no sea sobreescrito

        # Comprueba si la propiedad dependencias esta declarada
        if archivo.get("dependencias"):
            comprobar_dependencias(
                archivo.get("dependencias")
            )  # Le pasa las dependendias a la funcion comprobar dependencias
        ...
        if type(archivo.get("variables")) is dict:
            ...
            archivo_config["contiene_variables"] = (
                archivo.get("contiene_variables") or True
            )
            archivo_config["variables"] = archivo.get(
                "variables"
            )  # Almacena las varibles globales
        elif (
            type(archivo.get("contiene_variables")) is bool
        ):  # Comprueba si contiene variables es un boolean
            archivo_config["contiene_variables"] = archivo.get(
                "contiene_variables"
            )  # Guarda el valor ed contiene_variables en la configuracion global
        ...
        print(
            Colores.AMARILLO
            + "Aviso, ya se ha pasado antes otro archivo de configuración"
            + Colores.FINC
        )
...
def comprobar_configuracion(archivo):
    ...
    if archivo.get("archivo_configuracion") == True:
        archivo_configuracion(
            archivo
        )  # Pasa el json a la funcion encargada del archivo de configuracion
        ...
        if (
            not type(archivo.get("comandos")) is list
        ):  # En caso de que comandos no este definido lanza un error
            print(
                Colores.ROJO
                + "Error, es necesario que exista la propiedad comandos y que sea una lista"
                + Colores.FINC
            )  # Mensaje de error
            exit(1)  # Sale con el codigo de error 1
        ...
        renderizar_variables(
            archivo.get("comandos"),
            archivo.get("dependencias"),
            archivo.get("variables"),
            archivo.get("contiene_variables"),
        )
...
def configuracion_archivo(archivo):
    try:
        ...
        if type(json) is list:
            ...
            type(json) is dict
        ):  # En caso de ser un diccionario se llama a la funcion para comprobar su configuracion
        ...
        print(
            ...
            + archivo.name
            + Colores.ROJO
            + ", esta vacio o tiene un formato erroneo"
            + Colores.FINC
        )  # Mensaje error
        tipo_error.archivo(
            1
...
def comprobador_archivos(listado_archivos):
    try:
        ...
        if type(listado_archivos) is str:
            listado_archivos = [
                listado_archivos
            ]  # Convierte listado_archivos en una lista

        if (
            not type(listado_archivos) is list
        ):  # Si listado_archivos no es una lista lanza una excepcion
        ...
        for archivo in listado_archivos:  # Recorre todos los archivos
            ...
            try:
                with open(
                    archivo, "r"
                ) as fichero:  # Abre el archivo config.json en modo lectura
                    configuracion_archivo(
                        fichero
                    )  # LLama a la funcion que se encarga de convertir los datos del archivo en un json
                # Muestra un mensaje indicando la finalizacion del archivo
                print(
                    Colores.VERDE
                    + "Finalizada ejecucion el archivo: "
                    + Colores.AZUL
                    + archivo
                    + Colores.FINC
                )
...
if __name__ == "__main__":
    ...
    if len(argv) >= 2:  # Comprueba si se ha introducido algun elemento por parametro
        archivos = argv[
            1:
        ]  # En caso de haber introducido algun elemento, obtendra el array apartir de la segunda posicion
